...
def generate_variants() -> list[Variant]:
    ...
    for price_min, price_max in price_bands:
        for close_cap in close_caps:
            ...
            band_part = f"winner {price_min:.2f}-{price_max:.2f}"
            out.append(
                Variant(
                    key=f"M{seq:04d}",
                    label=f"{band_part}{close_part}",
                    price_min=price_min,
                    price_max=price_max,
            ...
            for vr in vol_ratios:
                for vlb in vol_lookbacks:
                    out.append(
                        Variant(
                            key=f"M{seq:04d}",
                            label=f"{band_part} vol{vlb}>={vr:.1f}x{close_part}",
                            price_min=price_min,
                            price_max=price_max,
                            close_max_usd=close_cap,
                            volume_ratio_min=vr,
            ...
            for mm in move_mins:
                for mlb in move_lookbacks:
                    out.append(
                        Variant(
                            key=f"M{seq:04d}",
                            label=f"{band_part} move{mlb}>={mm:.1f}{close_part}",
                            price_min=price_min,
                            price_max=price_max,
                            close_max_usd=close_cap,
                            volume_ratio_min=None,
                    ...
                    for mm in move_mins:
                        for mlb in move_lookbacks:
                            out.append(
                                Variant(
                                    key=f"M{seq:04d}",
                                    label=f"{band_part} vol{vlb}>={vr:.1f}x OR move{mlb}>={mm:.1f}{close_part}",
                                    price_min=price_min,
                                    price_max=price_max,
                                    close_max_usd=close_cap,
                                    volume_ratio_min=vr,
...
def summarize_variant(sample: list[WindowData], variant: Variant) -> tuple[dict[str, object], list[dict[str, object]]]:
    ...
    for rank, win in enumerate(sample, start=1):
        ...
        for idx, tick in enumerate(win.ticks):
            ...
            fill_t, fill_px = fill
            entry_px = fill_px
            if fill_px < variant.price_min - 1e-12:
                reason = "below_band_tplus2"
                break
            if fill_px > variant.price_max + 1e-12:
                reason = "above_band_tplus2"
                break
            traded = True
        ...
        detail_rows.append(
            {
                ...
                "traded": int(traded),
                "side": side,
                "fill_t": "" if fill_t is None else fill_t,
    ...
    row = {
        ...
        "notes": "Fresh winner-side mid-band search, $1 notional, fill booked at t+2 within the 35c-70c style band.",
    }
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Fresh winner-side mid-band signal search on BTC 5m public windows.")
    parser.add_argument(
        "--input-dir",
        type=Path,
    ...
    parser.add_argument(
        "--out-summary",
        type=Path,
        default=repo_root / "reports" / "winner_midband_search_sheet.csv",
    )
    parser.add_argument(
        "--out-detail",
        type=Path,
        default=repo_root / "reports" / "winner_midband_search_detail.csv",
    )
