...
class _FakeClob:
    ...
    def get_open_orders_for_asset(self, token: TokenMarket) -> list[dict[str, object]]:  # noqa: ANN201
        return [
            dict(row)
    ...
    def drop_open_order(self, order_id: str) -> None:
        ...
        self.open_order_ids.discard(str(order_id))
...
def test_zero_avg_price_from_pm_uses_pending_order_price_for_cost() -> None:
    ...
    _finish_cycle(
        ...
        positions_seq=[
            [_positions_row(slug=runner.contract.slug, outcome="DOWN", token_id="down-token", size=4.081632653, avg_price=0.0)],
            [
                _positions_row(slug=runner.contract.slug, outcome="DOWN", token_id="down-token", size=4.081632653, avg_price=0.48),
                _positions_row(slug=runner.contract.slug, outcome="UP", token_id="up-token", size=4.081632653, avg_price=0.51),
            ],
        ],
    )
...
def test_bootstrap_amount_is_capped_by_max_shares_per_side() -> None:
    ...
    _finish_cycle(
        ...
        positions_seq=[
            ...
            [
                _positions_row(slug=runner.contract.slug, outcome="UP", token_id="up-token", size=15.0, avg_price=0.10),
                _positions_row(slug=runner.contract.slug, outcome="DOWN", token_id="down-token", size=5.0, avg_price=0.89),
            ],
        ],
    )
...
def test_failed_order_does_not_update_position_and_waits_before_retry() -> None:
    ...
    _finish_cycle(
        ...
        positions_seq=[
            ...
            [
                _positions_row(slug=runner.contract.slug, outcome="DOWN", token_id="down-token", size=4.0, avg_price=0.245),
                _positions_row(slug=runner.contract.slug, outcome="UP", token_id="up-token", size=4.0, avg_price=0.51),
            ],
        ],
    )
...
def test_zero_avg_on_smaller_side_still_places_balance_limit() -> None:
    ...
    clob = _tick(
        ...
        positions_seq=[
            [
                _positions_row(slug=runner.contract.slug, outcome="UP", token_id="up-token", size=10.0, avg_price=0.45),
                _positions_row(slug=runner.contract.slug, outcome="DOWN", token_id="down-token", size=5.0, avg_price=0.0),
            ]
        ],
    )
...
def test_duplicate_open_balance_orders_are_cancelled_and_no_new_order_placed() -> None:
    ...
    clob = _FakeClob()
    clob.open_orders = [
        {"id": "dup-1", "asset_id": "down-token", "side": "BUY", "price": 0.61, "original_size": 5.0, "size_left": 5.0},
        {"id": "dup-2", "asset_id": "down-token", "side": "BUY", "price": 0.60, "original_size": 5.0, "size_left": 5.0},
        {"id": "dup-3", "asset_id": "down-token", "side": "BUY", "price": 0.59, "original_size": 5.0, "size_left": 5.0},
    ]
    clob.open_order_ids = {"dup-1", "dup-2", "dup-3"}
    ...
    assert clob.calls == []
    assert len(clob.open_orders) == 3
    assert runner.pending_order is True
    assert runner.pending_order_id == "dup-1"


def test_active_open_order_blocks_second_balance_post() -> None:
    state = PositionState(
        spent_up=4.5,
        shares_up=10.0,
        spent_down=2.4,
    ...
    clob = _FakeClob()
    clob.open_orders = [
        {"id": "live-1", "asset_id": "down-token", "side": "BUY", "price": 0.61, "original_size": 5.0, "size_left": 5.0},
    ]
    clob.open_order_ids = {"live-1"}
...
def test_open_up_order_cancelled_when_hedge_requires_down() -> None:
    ...
    assert "bad-up" in clob.open_order_ids
    assert clob.calls == []
