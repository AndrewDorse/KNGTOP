...
def cycle_begin_hedge(runner: WindowRunner, *, side: str, price: float, shares: float) -> None:
    c = runner.cycle
        ...
        if clob is not None:
            ...
            if isinstance(payload, dict) and payload:
                ...
                try:
                    ...
                    pass
    token_id = runner.contract.up.token_id if side == "UP" else runner.contract.down.token_id
    for view in runner.open_orders.get(side, []):
        if view.side == side:
            return True
    for row in _filtered_open_orders_for_runner(runner, clob.get_open_orders() if clob else []):
...
def enforce_single_order(
    ...
    *,
    ...
    if keep_one is not None and cycle_is_idle(runner):
        adopt_open_order(runner, view=keep_one)
    return bool(runner_open_order_views(runner)) or cycle_is_busy(runner)


def adopt_open_order(runner: WindowRunner, *, view: OpenOrderView) -> None:
