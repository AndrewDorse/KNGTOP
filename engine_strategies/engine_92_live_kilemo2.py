...
def _can_place_limit_buy(runner: WindowRunner, side: str, price: float, cfg: KngtopConfig) -> bool:
    ...
    return balance_side == side and _balance_limit_improves_gap(state, side, price, cfg)
...
def _balance_limit_repair_action(
    ...
    *,
    ...
    state = _effective_state_with_pending(runner)
    side = _balance_required_side(state, cfg)
    if side is None:
        return None
    ...
    if not _can_buy(state, side, amount_usd, ask_px=target_price, cfg=cfg):
        return None
    if not _balance_limit_improves_gap(state, side, target_price, cfg):
        return None
    return BuyAction(
        side=side,
        ask_px=target_price,
        amount_usd=amount_usd,
        reason="balance_limit_repair",
...
def _choose_guarded_pnl_buy(
    ...
    *,
    ...
    if synced.total_deals == 0 and not runner.pending_order:
    ...
    if missing_side is not None:
        side = missing_side
    elif (balance_action := _balance_limit_repair_action(runner, up_ask=up_ask, down_ask=down_ask, cfg=cfg)) is not None:
    ...
    balance_side = _balance_required_side(state, cfg)
    if balance_side is not None and side != balance_side:
        _log_tag(
            "SKIP",
            slug=runner.contract.slug,
            side=side,
            reason="balance_side_required",
            required_side=balance_side,
            shares_up=f"{state.shares_up:.6f}",
            shares_down=f"{state.shares_down:.6f}",
...
def _maybe_guarded_pnl_buy(
    ...
    *,
    ...
    if action is None:
        return False
    limit_price = action.ask_px if action.reason == "balance_limit_repair" else _passive_limit_buy_price(action.ask_px)
