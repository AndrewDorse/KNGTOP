    ...
    *,
    ...
    if c.phase == lo.PHASE_WAIT_PRIMARY:
        ...
        if primary_on_book:
            ...
            if hedge_side is not None and hedge_side != c.primary_side:
                ...
                if _can_buy(confirmed, hedge_side, hedge_shares * hedge_price, ask_px=hedge_price, cfg=cfg):
                    lo.cycle_begin_hedge(runner, side=hedge_side, price=hedge_price, shares=hedge_shares)
    ...
    if c.phase == lo.PHASE_WAIT_HEDGE:
        if lo.order_on_clob(runner, clob=clob, order_id=c.hedge_order_id, side=c.hedge_side):
    ...
    if c.phase == lo.PHASE_WAIT_PM:
        ...
        if lo.tick_pm_stable(runner, up_shares=pos.shares_up, down_shares=pos.shares_down, now_ts=now_ts):
            ...
            runner.next_decision_ts = now_ts
            return False
        return True
...
def _send_limit_buy(
    *,
    ...
    lo.cycle_begin_primary(runner, side=side, price=price, shares=shares, reason=reason)
    ...
    try:
        payload = clob.limit_buy_shares(_token_for_side(runner, side), price=price, shares=shares, post_only=True)
        order_id = _extract_order_id(payload) if isinstance(payload, dict) else None
        if not order_id:
            lo.cycle_reset(runner)
