...
- `engine_85_live_kilemo2.py` + `engine_85_live_orders.py`: ONE trading path — bootstrap/hedge-only sends, single `_post_cycle_limit` CLOB entry, cycle closes only when PM fills + balanced (current push).
- `engine_86_live_kilemo2.py` + `engine_86_live_orders.py`: strict two-leg `OrderCycle` — one primary, confirm CLOB, one hedge, 5× PM stable, then next.
