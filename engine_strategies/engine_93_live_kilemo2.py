...
def _can_place_limit_buy(runner: WindowRunner, side: str, price: float, cfg: KngtopConfig) -> bool:
    ...
    if shares <= 0.0 or cost + 1e-12 < LIMIT_MIN_USD:
        return False
    return _can_buy(_effective_state_with_pending(runner), side, cost, ask_px=price, cfg=cfg)
...
def _imbalance_limit_repair_action(
    state: PositionState,
    *,
    up_ask: float,
    down_ask: float,
    ...
    if not state.both_sides_traded():
        return None
    current_gap = abs(state.shares_up - state.shares_down)
    if current_gap <= _configured_max_share_gap(cfg) + 1e-12:
        return None
    side = "UP" if state.shares_up < state.shares_down else "DOWN"
    current_avg = _avg_for_side(state, side)
    if current_avg <= 1e-12:
        return None
    quoted_ask = up_ask if side == "UP" else down_ask
    target_price = min(_passive_limit_buy_price(quoted_ask), current_avg - 0.01)
    if target_price <= 0.0:
        return None
    amount_usd = _limit_order_cost(target_price)
    if not _can_buy(state, side, amount_usd, ask_px=target_price, cfg=cfg):
        return None
    projected_gap = _share_gap_after_buy(state, side, target_price, amount_usd)
    if projected_gap >= current_gap - 1e-12:
        return None
    return BuyAction(side=side, ask_px=target_price, amount_usd=amount_usd, reason="imbalance_limit_repair", enforce_avg_cap=False)
...
def _choose_guarded_pnl_buy(
    ...
    *,
    ...
    if missing_side is not None:
        side = missing_side
    elif (imbalance_action := _imbalance_limit_repair_action(state, up_ask=up_ask, down_ask=down_ask, cfg=cfg)) is not None:
        return imbalance_action
...
def _maybe_guarded_pnl_buy(
    ...
    *,
    ...
    if action is None:
        return False
    limit_price = action.ask_px if action.reason == "imbalance_limit_repair" else _passive_limit_buy_price(action.ask_px)
